...
// ─────────────────────────────────────────────────────────────────────────────
// CAMBIOS v4:
//   - Sin input de pago (los pagos se registran desde Recurrentes)
//   - linked_account_id muestra el ícono del subtype correcto
//   - linkedRecurring muestra el ícono de la cuenta del recurrente
//   - debtPayments muestra el ícono de la cuenta en cada pago
// ─────────────────────────────────────────────────────────────────────────────
...
import { fmt, pct, ACCOUNT_SUBTYPES } from '../lib/constants'
...
export default function Debts() {
  ...
  const totalMonthly = useMemo(() => debts.reduce((s, d) => s + (d.monthly_payment || 0), 0), [debts])

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <SectionHeader
        title={t.debts}
        action={<Btn size="sm" variant="primary" onClick={() => openModal('debt')}>+ {t.addDebt}</Btn>}
      />
      ...
      {debts.length > 0 && (
        <div className="g4">
          {[
            ...
            { label: 'Cuota mensual total', value: fmt(totalMonthly), color: 'var(--orange)' },
          ].map(m => (
      ...
      {/* Banner informativo — cómo pagar */}
      {debts.length > 0 && (
        <div style={{
          background: '#4f7cff10', border: '1px solid #4f7cff33',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>
                Ve a <strong>Recurrentes</strong> y vincula el pago mensual a esta deuda.
                Al marcarlo pagado, se abona automáticamente aquí.
              </div>
            </div>
          </div>
...
function DebtCard({ debt }) {
  ...
  // Cuenta de débito vinculada — v4: muestra ícono del subtype
  const linkedAccount = getAccount(debt.linked_account_id)
  const linkedAccCfg = linkedAccount ? ACCOUNT_SUBTYPES[linkedAccount.subtype] : null

  // Pago recurrente vinculado a esta deuda
  const linkedRecurring = recurring.find(r => r.linked_debt_id === debt.id)
  const recAccount = linkedRecurring ? getAccount(linkedRecurring.account_id) : null
  const recAccCfg = recAccount ? ACCOUNT_SUBTYPES[recAccount.subtype] : null

  // Últimos pagos automáticos
      ...
      <Card accent={isCompleted ? 'var(--green)44' : undefined}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
          <div>
            ...
            {/* Cuenta vinculada con ícono del subtype */}
            {linkedAccount && (
              <div style={{
                fontSize: 11, color: 'var(--muted)', marginTop: 2,
                display: 'flex', alignItems: 'center', gap: 4
              }}>
                {linkedAccCfg?.icon} {linkedAccount.name} — cuenta de pago
              </div>
            )}
            {debt.notes && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
            <div className="mono" style={{ fontSize: 22, color: isCompleted ? 'var(--green)' : 'var(--red)' }}>
              {fmt(remaining)}
            </div>
            <div style={{ display: 'flex', gap: 5 }}>
              <Btn size="xs" variant="ghost" onClick={() => setEditing(true)}>✏️ Editar</Btn>
              {!confirmDel ? (
                <Btn size="xs" variant="danger" onClick={() => setConfirmDel(true)}>✕</Btn>
              ) : (
                <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                  <span style={{ fontSize: 11, color: 'var(--muted)' }}>¿Confirmar?</span>
                  <Btn size="xs" variant="danger" onClick={() => deleteDebt(debt.id)}>Sí</Btn>
        ...
        <div className="g2" style={{ marginBottom: 14 }}>
          {[
            { label: t.paidAmount, value: fmt(paid), sub: `${paidPct}% del total`, color: 'var(--green)' },
            { label: t.remaining, value: fmt(remaining), sub: monthsLeft > 0 ? `~${monthsLeft} pagos estimados` : 'Sin cuota', color: remaining > 0 ? 'var(--red)' : 'var(--green)' },
            { label: t.estimatedPayoff, value: monthsLeft > 0 ? payoffDate.toLocaleDateString('es-CA', { year: 'numeric', month: 'short' }) : '—', sub: monthsLeft > 0 ? `${monthsLeft} meses` : '', color: 'var(--purple)' },
            { label: t.totalInterest, value: fmt(totalInterest), sub: 'estimado (cuota fija)', color: 'var(--yellow)' },
          ].map(m => (
            <div key={m.label} style={{
              ...
              <div className="lbl">{m.label}</div>
              <div className="mono" style={{ fontSize: 14, color: m.color, marginTop: 2 }}>{m.value}</div>
        ...
        {linkedRecurring ? (
          <div style={{
            ...
            <div>
              ...
              <div style={{
                ...
                {recAccount && (
                  <span style={{ color: recAccount.color }}>
                    · {recAccCfg?.icon} {recAccount.name}
                  </span>
                )}
              </div>
            </div>
          ...
          <div style={{
            background: 'var(--muted)0a', border: '1px solid var(--border)',
            borderRadius: 'var(--radius-sm)', padding: '10px 14px', marginBottom: 12,
            fontSize: 12, color: 'var(--muted)', display: 'flex', alignItems: 'center', gap: 8,
          }}>
            <span>💡</span>
            <span>
              Sin recurrente vinculado. En <strong style={{ color: 'var(--text)' }}>Recurrentes → Editar</strong>,
              activa "Vincular deuda" para que los pagos se registren automáticamente.
            </span>
          </div>
        )}
        ...
        {isCompleted && (
          <div style={{
            ...
            color: 'var(--green)', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 16,
          }}>
